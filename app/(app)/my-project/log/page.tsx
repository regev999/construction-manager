...
export default function MyProjectLog() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    if (user.id === 'dev-user') {
      ...
      if (saved) {
        ...
        router.replace(`/projects/${p.id}/log`)
      } else {
    ...
    supabase.from('projects').select('id')
      ...
      .then(({ data, error: err }) => {
        ...
        router.replace(`/projects/${data.id}/log`)
      })
  }, [user])

  if (error) return <div className="flex items-center justify-center h-64 text-gray-400 text-sm">לא נמצא פרויקט</div>
